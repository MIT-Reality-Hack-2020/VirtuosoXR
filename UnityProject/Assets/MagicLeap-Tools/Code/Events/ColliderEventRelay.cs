...
namespace MagicLeapTools
{
    ...
    public class ColliderEventRelay : MonoBehaviour
    {
        ...
        {
            events.OnTriggerEntered?.Invoke(other);
        }

        private void OnTriggerStay(Collider other)
        {
            events.OnTriggerStayed?.Invoke(other);
        }

        private void OnTriggerExit(Collider other)
        {
            events.OnTriggerExited?.Invoke(other);
        }

        private void OnCollisionEnter(Collision collision)
        {
            events.OnCollisionEntered?.Invoke(collision);
        }

        private void OnCollisionStay(Collision collision)
        {
            events.OnCollisionStayed?.Invoke(collision);
        }

        private void OnCollisionExit(Collision collision)
        {
            events.OnCollisionExited?.Invoke(collision);
        }
    }
}