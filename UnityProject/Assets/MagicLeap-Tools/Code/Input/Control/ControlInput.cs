...
namespace MagicLeapTools
{
    public class ControlInput : MonoBehaviour
    {
        ...
        private void Initialize(MLInputController control)
        {
            ...
            Control = control;
            Connected = true;
            events.OnControlConnected?.Invoke();
        ...
        private void Update()
        {
            ...
            if (!Touch && Control.Touch1Active)
            {
                ...
                {
                    events.OnDoubleTap?.Invoke(TouchValue);
                }
                ...
                events.OnTouchDown?.Invoke(TouchValue);
            }
                    ...
                    if (delta > _minForceDelta)
                    {
                        if (_activeTouch.z > TouchValue.z)
                        {
                            ...
                            if (!ForceTouch && _activeTouch.z >= _forceTouchDownThreshold)
                            {
                                ...
                                events.OnForceTouchDown?.Invoke();
                            }
                        }
                        else
                        {
                ...
                if (!_wasForceTouched)
                {
                    ...
                    if (!TouchMoved)
                    {
                        ...
                        if (movedFromInitialTouchDistance > _touchBeganMovingThreshold)
                        {
                            ...
                            events.OnTouchBeganMoving?.Invoke();
                        }
                    }
                    ...
                    if (TouchMoved)
                    {
                        ...
                        {
                            //moved:
                            events.OnTouchMove?.Invoke(TouchValue);

                            //radial move:
                            float angleDelta = _activeTouch.w - TouchValue.w;
                            if (events.OnTouchRadialMove != null)
                            {
                                TouchRadialDelta = angleDelta;
                                events.OnTouchRadialMove?.Invoke(angleDelta);
                            }
            ...
            if (!Control.Touch1Active && Touch)
            {
                    ...
                    else
                    {
                        ...
                        if (events.OnTapped != null)
                        {
                            ...
                            if (durationFromTouchStart < _maxTapDuration)
                            {
                                ...
                                events.OnTapped?.Invoke(direction);
                            }
                        }
                    }
                }
                ...
                events.OnTouchUp?.Invoke(TouchValue);
            ...
            if (TriggerValue != Control.TriggerValue)
            {
                ...
                if (TriggerValue == 0)
                {
                    events.OnTriggerPressBegan?.Invoke();
                }

                //trigger moved:
                events.OnTriggerMove?.Invoke(Control.TriggerValue - TriggerValue);

                //trigger released:
                if (Control.TriggerValue == 0)
                {
                    events.OnTriggerPressEnded?.Invoke();
                }
        ...
        private void HandleControlDisconnected(byte controlId)
        {
            ...
            Connected = false;
            Control = null;
            events.OnControlDisconnected?.Invoke();
